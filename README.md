...
**The library rail.** Since 1.9.0 the library page carries a left rail listing its class groups, with **All decks** at the top, in the same order as the list itself — most recently taught class first, not alphabetical. It appears only when there are **two or more groups** (a rail with one destination is noise) and only when **TBT Hub ≥ 1.3.0** is active: Hub owns `tbt-rail.css` and therefore the rail's appearance, while Swipe adds nothing but where it sits. With Hub deactivated no rail markup is emitted at all and the library renders as the stacked list it has always been; the generator page never loads the rail stylesheet either.

The rail **filters, it does not navigate**. Clicking a class hides the other groups and retitles the heading to that class; clicking **All decks** brings them all back and restores `Your decks`. Nothing is scrolled to, the URL is untouched and no history entry is added — a later release adds a destination whose decks are not in the page at all, which scrolling could not reach. The **Create new deck** button sits above the split and stays put whichever filter is on. The choice is not remembered: after editing a deck and returning, the library opens on **All decks**. Below 782px the rail becomes a horizontal strip above the list rather than a pinned column.
